...
function proxyInnerScreenRender() {
  if (!InnerScreen) {
    console.log("__RNIDE_INTERNAL", "InnerScreen is not defined, skipping screens proxy");
    return;
  }
  ...
  if (!origRender) {
    console.log("__RNIDE_INTERNAL", "InnerScreen.render is not defined, skipping screens proxy");
    return;
  }
...
try {
  ...
  console.log("__RNIDE_INTERNAL", "Failed to proxy InnerScreen render function", e);
}
