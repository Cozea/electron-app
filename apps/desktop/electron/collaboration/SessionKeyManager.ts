...
export class SessionKeyManager {
  ...
  async supplyWaitingDevices(sessionId: string, roomKeyBase64: string, keyVersion: number): Promise<void> {
    const waiting = await this.gateway.post<Array<{ principalId: string; deviceId: string; publicKeyJwk: string; keyVersion: number }>>("/collab/v2/keys", { operation: "waitingDevices", sessionId, keyVersion })
    ...
    for (const recipient of waiting) {
      ...
      await this.gateway.post("/collab/v2/keys", { operation: "share", sessionId, recipientUserId: recipient.principalId, keyVersion, wrappedKey: wrapped.wrappedKey, wrapAlgorithm: wrapped.wrapAlgorithm, senderPublicKeyJwk: wrapped.senderPublicKeyJwk })
    }
  }
