...
export class AuthorizedRepositoryDownloader {
  ...
  private async run(projectId: string, slug: string, signal: AbortSignal): Promise<LocalWorkspaceDTO> {
    ...
    try {
      stage("authorizing")
      // Repository identity and the short-lived credential are returned atomically
      // from the gateway, so there is no stale binding/TOCTOU window here.
      ...
      if (
        ...
        throw new Error("Repository authorization is invalid or expired; retry the download")
      }
      ...
      const workspace = await this.deps.allocate(projectId, slug, async directory => {
        const git = async (args: string[], remote = false) => {
          ...
          check()
          if (!result.success && args[0] !== "checkout-index") {
            throw new Error(remote ? "GitHub download failed. Check repository access and retry." : "Repository preparation failed; local recovery was retained")
          }
        ...
        if (entries.includes(".git")) {
          ...
          receipt = await fs.readFile(receiptPath, "utf8").then(value => JSON.parse(value) as typeof receipt).catch((error: NodeJS.ErrnoException) => { if (error.code === "ENOENT") return null; throw error })
        } else {
