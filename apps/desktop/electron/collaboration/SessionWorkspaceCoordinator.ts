...
export class SessionWorkspaceCoordinator {
  ...
  async readReviewedImport(sessionId: string, selected: Array<{ path: string; reviewHash: string }>, accessToken: string): Promise<CollaborationTextChange[]> {
    ...
    const binding = await this.getBinding(sessionId)
    if (!binding || authority.session.createdByPrincipalId !== authority.principalId || authority.session.publishedThroughSequence !== 0) throw new Error("Local import is available only while starting the session")
  ...
  async prepareCommit(input: PrepareCollaborationCommitInput): Promise<PreparedCollaborationCommit> {
    return this.serial(input.sessionId, async () => {
      ...
      const { session } = authority
      if (session.commitLeasePrincipalId !== authority.principalId || session.status !== "commit_preparing" || !session.commitLeaseExpiresAt || session.commitLeaseExpiresAt <= this.now() ||
  ...
  async pushPrepared(sessionId: string, accessToken: string): Promise<PreparedCollaborationCommit> {
    return this.serial(sessionId, async () => {
      ...
      if (session.status !== "pushing" || session.commitLeasePrincipalId !== authority.principalId || !session.commitLeaseExpiresAt || session.commitLeaseExpiresAt <= this.now() ||
