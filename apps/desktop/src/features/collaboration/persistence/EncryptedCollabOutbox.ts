...
class IndexedDbOutboxBackend implements OutboxBackend {
  ...
  private async transaction<T>(
    ...
    return await new Promise<T>((resolve, reject) => {
      ...
      transaction.onabort = () => reject(transaction.error ?? new Error("Collaboration outbox transaction aborted"))
      transaction.oncomplete = () => ready ? resolve(result) : reject(new Error("Collaboration outbox transaction returned no result"))
      // An individual request succeeding does not mean the transaction committed.
      operation(transaction.objectStore(STORE_NAME), value => { result = value; ready = true }, reject)
    })
  }
...
export class EncryptedCollabOutbox {
  ...
  constructor(factory?: IDBFactory | null) {
    // null explicitly selects the test-only memory backend. Production must
    // pause visibly if durable storage is unavailable instead of losing edits.
    if (factory === null) this.backend = new MemoryOutboxBackend()
    else {
      const durableFactory = factory ?? (typeof indexedDB === "undefined" ? undefined : indexedDB)
      if (!durableFactory) throw new Error("Durable collaboration storage is unavailable")
      this.backend = new IndexedDbOutboxBackend(durableFactory)
    }
  }
