...
export function StructuralConflictDialog({ publicSessionId, canEdit, onClose }: StructuralConflictDialogProps) {
  ...
  return (
    <UnifiedModal
      ...
      title={t("collab.pathConflicts")}
      size="xl"
    >
      <div className="space-y-4">
        <p className="text-sm text-muted-foreground">{t("collab.chooseWhereEachFileBelongsOr")}</p>
        <div className="space-y-3" aria-busy={busy}>
          ...
          {!canEdit && <p className="text-sm text-muted-foreground">{t("collab.viewersCanReviewConflictsEditAccess")}</p>}
          {!page && busy && <p>{t("collab.loadingPaths")}</p>}
          {page?.conflicts.length === 0 && <p>{t("collab.noPathConflictsOnThisPage")}</p>}
          <div className="max-h-80 space-y-4 overflow-auto">
            {page?.conflicts.map((conflict) => <fieldset key={conflict.fileId} disabled={busy || !canEdit} className="space-y-2 rounded border p-3">
              ...
              {conflict.textPreview !== undefined && <details><summary className="text-xs">{t("collab.retainedContentPreviewUpTo2")}</summary>
              ...
              {choices[conflict.fileId] && <>
                {choices[conflict.fileId]!.action === "delete" ? <p className="text-sm">{t("collab.thisFileWillBeDeletedFrom")}</p> : <>
                ...
                })}>{t("collab.applyResolution")}</Button>
              </>}
            </fieldset>)}
          </div>
          <div className="flex gap-2"><Button variant="outline" disabled={busy} onClick={() => void run({ action: "list" })}>{t("collab.refresh")}</Button>
            ...
            <Button variant="outline" disabled={busy} onClick={onClose}>{t("collab.close")}</Button></div>
        </div>
      </div>
    </UnifiedModal>
  )
