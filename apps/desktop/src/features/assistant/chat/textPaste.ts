...
export const PASTED_TEXT_ATTACHMENT_LINE_THRESHOLD = 100;
...
export function isPasteAsTextShortcut(
  event: KeyboardShortcutModifiers,
  macPlatform: boolean,
): boolean {
  return (
    (event.key === undefined || event.key.toLowerCase() === "v") &&
    Boolean(event.shiftKey) &&
    !event.altKey &&
    (macPlatform ? Boolean(event.metaKey) && !event.ctrlKey : Boolean(event.ctrlKey) && !event.metaKey)
  );
}

/**
 * Large clipboard text becomes a file so an agent can inspect it selectively.
 * The threshold is byte-based or line-based: character counts substantially understate the
...
export function pastedTextDisposition(input: {
  ...
  return input.wouldExceedInputLimit ||
    lines >= PASTED_TEXT_ATTACHMENT_LINE_THRESHOLD ||
