...
import { serializeAssistantCitation } from "@/lib/assistantCitations";
import { Link, useNavigate } from "@/lib/router";
import { LuPencil, LuQuote, LuX } from "react-icons/lu";
...
} from "@/features/assistant/composerInlineChip";
...
export function AssistantCitationChip({
  ...
  const composerSourceLink = (
    ...
    >
      <LuQuote aria-hidden="true" className={COMPOSER_INLINE_CHIP_ICON_CLASS_NAME} />
    ...
    >
      ...
      {commentEditor ? (
        <Popover open={commentEditor.open} onOpenChange={commentEditor.onOpenChange}>
          ...
          >
            <LuPencil aria-hidden="true" className="size-[0.85em]" />
          </PopoverTrigger>
          {commentEditor.open ? (
            <PopoverPopup
      ...
      {onRemove ? (
        ...
        >
          <LuX aria-hidden="true" className="size-[0.85em]" />
        </button>
      ) : null}
    </span>
  );
