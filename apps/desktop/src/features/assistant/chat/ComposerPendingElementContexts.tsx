...
export interface ComposerPendingElementContextsProps {
  contexts: ReadonlyArray<ElementContextDraft>
  onRemove: (contextId: string) => void
  className?: string
}

export interface ComposerPendingElementContextChipProps {
      ...
      <TooltipTrigger
        render={
          <span className={cn(COMPOSER_INLINE_CHIP_CLASS_NAME, "pr-1")}>
            <LuMousePointerClick className={cn(COMPOSER_INLINE_CHIP_ICON_CLASS_NAME, "size-3.5")} />
            <span className={COMPOSER_INLINE_CHIP_LABEL_CLASS_NAME}>{label}</span>
            {sourceLabel ? (
              <span className="select-none text-2xs font-normal leading-tight text-muted-foreground/85">
                {sourceLabel}
              </span>
            ) : null}
            <button
