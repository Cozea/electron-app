...
export class TextRevealController {
  ...
  getSnapshot = (id: string): TextRevealSnapshot => this.entries.get(id)?.snapshot ?? EMPTY_SNAPSHOT
  ...
  private update(message: ChatMessage, immediate: boolean): void {
    ...
    const snapshotChanged = arrival?.snapshot !== undefined &&
      arrival.snapshot.revision !== entry.snapshotRevision
    if (snapshotChanged) {
      entry.target = arrival.snapshot!.text
      entry.shown = entry.target.length
      entry.boundaries = []
  ...
  private advance(id: string, entry: RevealEntry, now: number): void {
    ...
    const boundaries = entry.boundaries.filter((end) => end > entry.shown)
    ...
    for (const batch of entry.arrival?.batches ?? []) {
      if (now - batch.receivedAt >= TEXT_REVEAL_MAX_DELAY_MS) next = Math.max(next, batch.end)
    }
    // Round overdue offsets up to a whole grapheme.
    if (next > entry.shown) {
      entry.shown = boundaries.find((end) => end >= next) ?? entry.target.length
    }
    if (entry.shown >= entry.target.length) this.finish(id, entry)
    else this.publish(id, entry)
  }
  ...
  private publish(id: string, entry: RevealEntry): void {
    ...
    if (entry.snapshot.displayText === displayText && entry.snapshot.isRevealing === isRevealing) return
