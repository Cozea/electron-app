// Adapted from pinned T3 f2df43a98bc42936dd2a031d832c8c4dae53398a.
...
export function resolveCodexArtifactTemplate(
  ...
  if (
    ...
    !/^artifact-template-[A-Za-z0-9_-]+$/.test(skillName) ||
...
export { appendCodexArtifactTemplateUsePrompt as appendTemplateUsePrompt };

export function templateFromHastProperties(
  ...
  return resolveCodexArtifactTemplate({
    artifact_kind: value("dataArtifactKind"),
    display_name: value("dataDisplayName"),
    gallery_kind: value("dataGalleryKind"),
    skill_directory: value("dataSkillDirectory"),
    skill_name: value("dataSkillName"),
  });
}
