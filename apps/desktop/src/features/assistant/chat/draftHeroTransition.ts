export const DRAFT_HERO_TRANSITION_ANIMATION_ID = "cozea-draft-hero-transition";
export const DRAFT_HERO_TRANSITION_DURATION_MS = 180;
export const DRAFT_HERO_TRANSITION_EASING = "cubic-bezier(0.4, 0, 0.2, 1)";
export const MOBILE_COMPOSER_VIEW_TRANSITION_NAME = "cozea-mobile-composer";
export const MOBILE_DRAFT_HEADLINE_VIEW_TRANSITION_NAME = "cozea-mobile-draft-headline";
...
export interface ResolveDraftHeroStateInput {
  readonly isLocalDraftThread?: boolean;
  readonly hasTimelineEntries: boolean;
  readonly isWorking: boolean;
  readonly draftHeroDockRequested?: boolean;
  readonly backgroundSubmissionPending?: boolean;
}

export function resolveDraftHeroState(input: ResolveDraftHeroStateInput): boolean {
  if (input.backgroundSubmissionPending) {
    return true;
  }
  const isLocalDraft = input.isLocalDraftThread ?? true;
  const dockRequested = input.draftHeroDockRequested ?? false;
  return isLocalDraft && !input.hasTimelineEntries && !input.isWorking && !dockRequested;
}
  ...
  await Promise.all([
    ...
    ...activeTransitions.map(async (animation) => {
      try {
        ...
        // A cancelled transition is safe to hand off.
      }
    }),
  ]);
}
