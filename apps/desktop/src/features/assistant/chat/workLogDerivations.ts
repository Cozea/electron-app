/**
 * The work log: one row per thing the agent did in a turn.
 *
 * An orchestration activity is a wire record — a kind, a timestamp and an
 * `unknown` payload whose shape depends on the provider that produced it.
 * Everything here exists to turn that into a row a person can read: pull out the
 * command, the changed files, the tool title, the status; collapse a tool's
 * started/updated/completed activities into the single row they describe; drop
 * the activities that are bookkeeping rather than work.
 *
 * That is most of the file, and it is why this used to live in `session-logic`
 * behind a four-line re-export: the payload readers outnumber everything else in
 * a session by two to one, and they are read by exactly one caller. Session
 * derivations that several callers share — the timeline, pending requests, plan
 * state, phase — stayed there, and this file imports the few pieces it needs
...
import {
  ...
  toRawToolCommand,
} from "./toolCommandDisplay";
import { asNumber, asRecord, asTrimmedString } from "./toolPayloadValues";

export type { WorkLogEntry } from "./session-logic";
