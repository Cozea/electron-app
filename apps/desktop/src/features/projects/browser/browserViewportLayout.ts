import type { PreviewViewportSetting, PreviewViewportSize } from "@cozea/contracts/t3/preview"
...
export function resolveFittedBrowserViewport(
  ...
  sourceContent: { readonly width: number; readonly height: number; readonly scale: number } | null,
  zoomFactor = 1,
...
export function resizeBrowserViewportFromRail(
  ...
  return resizeFreeformViewport(
    start,
    {
      x: direction.includes("west") ? startWidth - desiredWidth : desiredWidth - startWidth,
      y: direction.includes("north") ? startHeight - desiredHeight : desiredHeight - startHeight,
    },
...
export function resolveResponsiveBrowserViewportSize(
  ...
  const normalizedZoomFactor = normalizeZoomFactor(zoomFactor)
  return resizeFreeformViewport(
    { width: area.width / normalizedZoomFactor, height: area.height / normalizedZoomFactor },
    { x: 0, y: 0 },
  )
}
