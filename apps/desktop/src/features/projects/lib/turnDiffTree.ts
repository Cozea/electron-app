...
function readStat(file: ActivityFeedItem): TurnDiffStat | null {
...
export function summarizeTurnDiffStats(files: ReadonlyArray<ActivityFeedItem>): TurnDiffStat {
  return files.reduce(
    (acc, file) => {
...
export function buildTurnDiffTree(files: ReadonlyArray<ActivityFeedItem>): TurnDiffTreeNode[] {
  ...
  for (const file of files) {
    const segments = normalizePathSegments(file.filePath);
