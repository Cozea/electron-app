/**
 * Shell command tokenizer and program-name extraction, ported from the upstream
 * t3code timeline. Used to label a running command by the program it actually
 * invokes ("Running bun") rather than echoing the whole command line or falling
 * back to a generic "Running command".
 *
 * Returns null rather than guessing whenever the command cannot be parsed
 * confidently — an unterminated quote, an unknown wrapper flag, or a wrapper
 * option whose value is missing.
 */
