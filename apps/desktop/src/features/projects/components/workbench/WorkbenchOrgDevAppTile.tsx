...
export function WorkbenchOrgDevAppTile({
  ...
  const [workerApproved, setWorkerApproved] = useState(false)
  ...
  const [installedResolution, setInstalledResolution] = useState<
    undefined | null | { artifact: ResolvedOrgDevAppArtifact; error: string | null }
  >(undefined)
  ...
  useEffect(() => {
    ...
    void window.electronAPI.orgDevApp
      .getInstallation({ ref: tile.devAppRef })
      .then(async (result) => {
        if (cancelled) return
        ...
        const prepared = await window.electronAPI.orgDevApp.prepareInstalled({
          ref: tile.devAppRef,
        })
        if (cancelled) return
  ...
  const runtimeLocation = artifact?.parts.runtime?.location ?? null
  const runtimeStateScope = artifact?.parts.runtime?.state ?? null
  ...
  useEffect(() => {
    if (!artifact?.parts.worker || !tile.devAppRef || !workspaceId || runtimeStopped) return
  ...
  }, [artifact, laneId, originUrl, prepareAttempt, runtimeStopped, tile.devAppRef, tile.id, workspaceId])
  ...
  useEffect(
    () => () => {
      if (runtimeKind === "service") {
        void window.electronAPI.orgDevApp.releaseRuntime({
          contentHash: runtimeContentHash,
          publicationId: artifact?.publicationId ?? "",
          leaseId: tile.id,
        })
      } else if (hasPublishedWorker && tile.devAppRef && workspaceId) {
        void window.electronAPI.orgDevApp.releasePublishedRuntime({
          ref: tile.devAppRef,
          workspaceId,
          leaseId: tile.id,
        })
  ...
  const statusMessage = useMemo(() => {
    ...
    if (workerApprovalRequired) return "This DevApp includes code that can work with the current project."
    if (runtimeStopped && artifact?.runtimeKind === "service") return "Service DevApp stopped."
    if (!originUrl)
      return artifact?.runtimeKind === "service" ? "Starting Service DevApp…" : t("orgDevApp.open.preparing")
    return null
  }, [
      ...
      actions={
        artifact?.parts.runtime ? (
          <div className="flex items-center gap-1">
            ...
            ) : artifact.parts.worker && tile.devAppRef && workspaceId ? (
              <>
                <Button
                  type="button"
                  variant="ghost"
            ...
            {artifact.parts.worker && workerApproved && tile.devAppRef && workspaceId ? (
              <Button
                type="button"
                variant="ghost"
                size="sm"
              ...
              <Button
                ...
                onClick={() => {
                  void refreshFolderGrants()
                    ...
                    .catch((error) =>
                      setPrepareError(error instanceof Error ? error.message : "Failed to inspect folder access."),
                    )
                }}
              >
                Files
                  ...
                  <Button
                    ...
                    onClick={() => {
                      ...
                      void window.electronAPI.orgDevApp
                        ...
                        .catch((error) =>
                          setPrepareError(error instanceof Error ? error.message : "Failed to revoke folder access."),
                        )
                    }}
                  >
                    Revoke
