import type { PreviewViewportPresetId, PreviewViewportSetting } from "@cozea/contracts/t3/preview"
import type { PreviewAutomationResizeInput } from "@cozea/contracts/t3/previewAutomation"

// Runtime mirrors of the pinned generated contract values. Importing the
// generated schema module would execute Effect-v3-only Schema.Defect against
// Cozea's effect-smol runtime; types continue to come from the synced contract.
...
export const PREVIEW_VIEWPORT_PRESET_IDS = [
  ...
  "nest-hub",
  "nest-hub-max",
] as const satisfies ReadonlyArray<PreviewViewportPresetId>
...
// Keep this in Chromium DevTools' standard-device order. This is the pinned
// T3 catalog; dimensions are CSS viewport sizes from EmulatedDevices.ts.
const PREVIEW_VIEWPORT_PRESET_DEFINITIONS = {
  ...
  "pixel-7": { label: "Pixel 7", category: "Phone", detail: "412 × 915", width: 412, height: 915 },
  "samsung-galaxy-s8-plus": {
...
export const PREVIEW_VIEWPORT_PRESETS: ReadonlyArray<PreviewViewportPreset> =
  PREVIEW_VIEWPORT_PRESET_IDS.map((id) => ({ id, ...PREVIEW_VIEWPORT_PRESET_DEFINITIONS[id] }))
