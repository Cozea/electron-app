...
export class BrowserRecordingUnavailableError extends Error {
  ...
  constructor(tabId: string) {
    super(`Browser recording is unavailable for tab ${tabId}.`)
    this.tabId = tabId
  }
}

export class BrowserRecordingConflictError extends Error {
  override readonly name = "BrowserRecordingConflictError"
  readonly requestedTabId: string
  readonly activeTabId: string
  constructor(requestedTabId: string, activeTabId: string) {
    super(`Cannot record tab ${requestedTabId} while tab ${activeTabId} is already being recorded.`)
    this.requestedTabId = requestedTabId
    this.activeTabId = activeTabId
  }
}

export class BrowserRecordingCanvasUnavailableError extends Error {
  override readonly name = "BrowserRecordingCanvasUnavailableError"
  readonly tabId: string
  readonly width: number
  readonly height: number
  constructor(tabId: string, width: number, height: number) {
    super(`Browser recording canvas ${width}x${height} is unavailable for tab ${tabId}.`)
    this.tabId = tabId
    this.width = width
    this.height = height
  }
...
export class BrowserRecordingOperationError extends Error {
  override readonly name = "BrowserRecordingOperationError"
  readonly operation: BrowserRecordingOperation
  readonly tabId: string
  override readonly cause: unknown
  constructor(operation: BrowserRecordingOperation, tabId: string, cause: unknown) {
    super(`Browser recording operation ${operation} failed for tab ${tabId}.`, { cause })
...
interface BrowserRecordingStoreState {
  readonly activeTabIds: ReadonlySet<string>
  readonly replace: (tabIds: ReadonlySet<string>) => void
}

export const useBrowserRecordingStore = create<BrowserRecordingStoreState>()((set) => ({
  activeTabIds: new Set(),
  replace: (activeTabIds) => set({ activeTabIds }),
}))
...
function publishActiveRecordings(): void {
  useBrowserRecordingStore.getState().replace(new Set(activeRecordings.keys()))
}
  ...
  image.addEventListener(
    ...
    () => {
      ...
      recording.context.drawImage(
        image,
        (recording.canvas.width - targetWidth) / 2,
        (recording.canvas.height - targetHeight) / 2,
        targetWidth,
        targetHeight,
      )
    },
...
type PreviewBridge = NonNullable<NonNullable<typeof window.desktopBridge>["preview"]>
...
const recordingStartupCancelledError = (
  ...
  new BrowserRecordingOperationError("start-screencast", recording.tabId, cause)
...
const waitForRecordingStartupToSettle = async (recording: ActiveRecording): Promise<void> => {
  ...
  try {
    await Promise.race([
      ...
      new Promise<void>((_, reject) => {
        timeout = setTimeout(
          () =>
            reject(
              new Error(`Browser recording startup did not settle for tab ${recording.tabId}.`),
            ),
    ...
    throw new BrowserRecordingOperationError("wait-startup", recording.tabId, cause)
  } finally {
    if (timeout !== null) clearTimeout(timeout)
  }
}
...
const isStartupWaitTimeout = (error: unknown): error is BrowserRecordingOperationError =>
  error instanceof BrowserRecordingOperationError && error.operation === "wait-startup"
...
export async function startBrowserRecording(
  ...
  if (activeRecording) {
    if (activeRecording.lifecycle.phase === "recording") return activeRecording.startedAt
    throw new BrowserRecordingConflictError(tabId, activeRecording.tabId)
  }
  ...
  if (!context) throw new BrowserRecordingCanvasUnavailableError(tabId, canvas.width, canvas.height)
  ...
  let settleFirstFrame: ((outcome: "frame" | "cancelled") => void) | undefined
  ...
  const recording: ActiveRecording = {
    ...
    startupSettled,
    firstFrameSize,
    settleFirstFrameSize: (outcome) => settleFirstFrame?.(outcome),
    recorder: null,
    mimeType: null,
  ...
  try {
    try {
      ...
      clearActiveRecording(recording)
      throw new BrowserRecordingOperationError("subscribe-frames", tabId, cause)
    }
    try {
      await bridge.recording.startScreencast(tabId)
    } catch (cause) {
      if (!isRecordingStarting(recording)) throw recordingStartupCancelledError(recording, cause)
      clearActiveRecording(recording)
      throw new BrowserRecordingOperationError("start-screencast", tabId, cause)
    }
...
async function finalizeBrowserRecording(
  ...
  let result:
    | { readonly _tag: "Success"; readonly artifact: DesktopPreviewRecordingArtifact | null }
  ...
  try {
    ...
    try {
      ...
      throw new BrowserRecordingOperationError("stop-screencast", tabId, cause)
    }
    if (!recording.recorder || !recording.mimeType) {
      ...
      try {
        ...
        throw new BrowserRecordingOperationError("stop-media-recorder", tabId, cause)
      }
      try {
        ...
        throw new BrowserRecordingOperationError("save-artifact", tabId, cause)
      }
    }
  } catch (error) {
  ...
  if (result._tag === "Failure" && isStartupWaitTimeout(result.error)) throw result.error

  let cleanupError: BrowserRecordingOperationError | undefined
  try {
    await stopMediaRecorder(recording.recorder)
  } catch (cause) {
    cleanupError = new BrowserRecordingOperationError("stop-media-recorder", tabId, cause)
  } finally {
    clearActiveRecording(recording)
  }
...
const discardBrowserRecordingAfterStartup = async (
  bridge: PreviewBridge,
  recording: ActiveRecording,
): Promise<null> => {
  try {
    ...
    .catch((error) => {
      if (isStartupWaitTimeout(error) && activeRecordings.get(recording.tabId) === recording) {
        const cleanupAfterStartup = recording.startupSettled.then(() =>
          discardBrowserRecordingAfterStartup(bridge, recording),
        )
...
export async function discardBrowserRecording(tabId: string): Promise<void> {
  ...
  if (!recording) return
  const bridge = window.desktopBridge?.preview
  await bridge?.recording.stopScreencast(tabId).catch(() => undefined)
  await stopMediaRecorder(recording.recorder).catch(() => undefined)
  clearActiveRecording(recording)
}
