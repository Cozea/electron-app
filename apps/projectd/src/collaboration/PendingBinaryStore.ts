...
export class PendingBinaryStore {
  ...
  stage(intent: PendingBinaryIntent, bytes: Buffer): PendingBinaryRecord {
    ...
    const revisionId = `staged_${hash(Buffer.from(JSON.stringify([
      intent.path, intent.fileId, intent.baseRevisionId, intent.mode, contentHash,
    ])))}`
    const existing = this.readRecord(revisionId)
    if (existing) {
      // Do not silently bless a damaged previous write as a durable retry.
      this.readBytes(existing)
      return existing
    }
  ...
  readBytes(record: PendingBinaryRecord): Buffer {
    ...
    for (let index = 0; index < record.chunks; index++) {
      ...
      const bytes = Buffer.from(this.cipher.open(row.envelope, `binary-chunk:${record.revisionId}:${index}`), "base64")
      if (bytes.length !== Math.min(CHUNK_SIZE_BYTES, record.size - index * CHUNK_SIZE_BYTES)) {
