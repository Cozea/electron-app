// Adapted from T3 f2df43a98bc42936dd2a031d832c8c4dae53398a.
// Keep canonical state independent of Cozea presentation mapping.
...
// Legacy detail-only snapshots omit shell metadata. Keep that absence explicit;
// a native OrchestrationThread satisfies this type without conversion.
type DetailFields =
  | "messages"
  | "activities"
  | "proposedPlans"
  ...
  | "session"
  | "latestTurn";
export type DetailCheckpoint = Omit<
  OrchestrationCheckpointSummary,
  "checkpointTurnCount" | "checkpointRef"
> &
  Partial<Pick<OrchestrationCheckpointSummary, "checkpointTurnCount" | "checkpointRef">>;
export type ThreadDetailState = Omit<Pick<OrchestrationThread, DetailFields>, "checkpoints"> & {
  readonly checkpoints: ReadonlyArray<DetailCheckpoint>;
} & Partial<Omit<OrchestrationThread, DetailFields>>;

export type ThreadDetailReducerResult =
  | { readonly kind: "updated"; readonly thread: ThreadDetailState }
  | { readonly kind: "deleted" }
  | { readonly kind: "unchanged" };

/** Terminal lifecycle closes unfinished text items without requiring a final flush. */
export function settleMessageStreams(
  messages: ReadonlyArray<OrchestrationMessage>,
  turnId: TurnId | null,
  completedAt: string,
): ReadonlyArray<OrchestrationMessage> {
  const matches = (message: OrchestrationMessage) =>
    message.role === "assistant" &&
    message.streaming &&
    (turnId === null || message.turnId === turnId || message.turnId === null);
  if (!messages.some(matches)) return messages;
  return messages.map((message) =>
    matches(message) ? { ...message, streaming: false, updatedAt: completedAt } : message,
  );
}
...
const checkpointOrder = O.mapInput(
  O.Number,
  (cp: DetailCheckpoint) => cp.checkpointTurnCount ?? Number.MAX_SAFE_INTEGER,
);
...
export function applyThreadDetailEvent(
  ...
  switch (event.type) {
    ...
    case "thread.message-sent": {
      // Buffered provider deltas still contribute text after settlement, but
      // cannot restart a terminal turn or mark an older turn's item live.
      const sameTurn = thread.latestTurn?.turnId === event.payload.turnId;
      const terminalTurn = sameTurn && thread.latestTurn?.state !== "running";
      const otherTurn = event.payload.turnId !== null && thread.latestTurn !== null && !sameTurn;
      const terminalSession =
        thread.session !== null && settledTurnStateForSessionStatus(thread.session.status) !== null;
      const streaming = event.payload.streaming && !terminalTurn && !otherTurn && !terminalSession;
      ...
      const settlesTurn = !streaming && !turnStillRunning;
...
function rebindCheckpointAssistantMessage(
  checkpoints: ReadonlyArray<DetailCheckpoint>,
  turnId: TurnId,
  messageId: MessageId,
): ReadonlyArray<DetailCheckpoint> {
