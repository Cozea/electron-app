import { isDevAppCapability, type DevAppCapability, type DevAppGrant } from "./devAppCapabilities"
...
/**
 * Bootstrap for the package-private channel between a DevApp view and its own worker.
 *
 * This is deliberately the same protocol as the worker/host channel rather than a second
 * independently versioned API. Main creates the opaque connection id and transfers one end
 * of the channel to each process; neither side may select another worker or protocol version.
 */
export interface DevAppWorkerViewPortBootstrap {
...
export function createDevAppWorkerViewPortBootstrap(
  connectionId: string,
  protocolVersion: number,
): DevAppWorkerViewPortBootstrap {
  if (!isBoundedString(connectionId, MAX_ID_LENGTH)) {
    throw new Error("The DevApp view connection id is invalid.")
  }
  if (!supportsDevAppWorkerProtocolVersion(protocolVersion)) {
    throw new RangeError(`Unsupported DevApp worker protocol ${String(protocolVersion)}.`)
  }
  return {
    kind: "cozea-devapp-view-port",
    protocolVersion,
...
/** Validates bootstrap metadata in the guest preload before exposing a transferred port. */
export function parseDevAppWorkerViewPortBootstrap(
  value: unknown,
): DevAppWorkerViewPortBootstrap | null {
  if (!isPlainObject(value) || value.kind !== "cozea-devapp-view-port") return null
  if (!isBoundedString(value.connectionId, MAX_ID_LENGTH)) return null
  if (!supportsDevAppWorkerProtocolVersion(value.protocolVersion)) return null
  const supported = value.supportedProtocolVersions
  if (
    !isPlainObject(supported) ||
    supported.min !== DEV_APP_WORKER_PROTOCOL_MIN_VERSION ||
    supported.max !== DEV_APP_WORKER_PROTOCOL_VERSION
  ) {
    return null
  }
  return {
