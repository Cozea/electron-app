...
export async function handleCollaborationKeys(request: Request, env: Env): Promise<Response> {
  ...
  if (operation !== 'initialize' && operation !== 'share' && operation !== 'waitingPrincipals' && operation !== 'rotationStatus' && operation !== 'beginRotation') throw new Error('Unknown session key operation')
  const client = new ConvexHttpClient(env.CONVEX_URL)
  client.setAuth(token)
  const result = operation === 'waitingPrincipals' || operation === 'rotationStatus'
