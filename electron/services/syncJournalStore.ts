...
interface SyncJournalStateRecord {
  journalHead: number
  pendingOps: number
...
const syncJournalStateByProject = new Map<string, SyncJournalStateRecord>()
...
function createDefaultSyncJournalState(): SyncJournalStateRecord {
  return {
    journalHead: 0,
    pendingOps: 0,
    lastAckedAt: null,
...
function getSyncJournalStateRecord(projectId: string): SyncJournalStateRecord {
  const existing = syncJournalStateByProject.get(projectId)
  if (existing) return existing
  const created = createDefaultSyncJournalState()
  syncJournalStateByProject.set(projectId, created)
  return created
}

function snapshotSyncJournalState(projectId: string, state?: SyncJournalStateRecord): {
  projectId: string
  journalHead: number
  pendingOps: number
  lastAckedAt: number | null
  ...
  const replica = state ?? getSyncJournalStateRecord(projectId)
  return {
    projectId,
    journalHead: replica.journalHead,
...
function getSyncJournalRecordSnapshot(projectId: string): ReturnType<typeof snapshotSyncJournalState> {
  const normalizedProjectId = String(projectId)
  const replica = getSyncJournalStateRecord(normalizedProjectId)
  replica.pendingOps = (queuedOpsByProject.get(normalizedProjectId) ?? []).length
  replica.lastStateVector = replica.journalHead - replica.pendingOps
  syncJournalStateByProject.set(normalizedProjectId, replica)
  return snapshotSyncJournalState(normalizedProjectId, replica)
}

export function getSyncJournalStateSnapshot(projectId: string): SyncJournalStatePayload {
  const snapshot = getSyncJournalRecordSnapshot(projectId)
  return {
...
function applyPathHead(replica: SyncJournalStateRecord, op: SyncOpRecord): void {
  if (op.kind === 'delete') {
...
export function enqueueSyncOps(projectId: string, incomingOps: SyncOpRecord[]): {
  ...
  rejected: number
  journalState: ReturnType<typeof snapshotSyncJournalState>
} {
  const normalizedProjectId = String(projectId)
  ...
  replica.journalHead += accepted
  replica.pendingOps = queue.length
  replica.lastStateVector = replica.journalHead
  ...
  return {
    ...
    acceptedOpIds,
    rejected,
    journalState: snapshotSyncJournalState(normalizedProjectId, replica),
  }
}
...
function getSyncStatePath(): string {
  return path.join(app.getPath('userData'), 'sync-journal-state.json')
}

function getSyncStateDbPath(): string {
  return path.join(app.getPath('userData'), 'sync-journal-state.sqlite')
}
...
let syncJournalDb: DatabaseSync | null = null
let syncJournalDbInitFailed = false

function getSyncJournalDb(): DatabaseSync | null {
  if (syncJournalDb) return syncJournalDb
  if (syncJournalDbInitFailed) return null
  ...
  try {
    ...
    `)
    syncJournalDb = db
    return syncJournalDb
  } catch (error) {
    syncJournalDbInitFailed = true
    console.warn('[Sync] Failed to initialize SQLite sync journal, falling back to JSON:', error)
    return null
  }
}

function normalizeSyncJournalRecord(value: Partial<SyncJournalStateRecord> | null | undefined): SyncJournalStateRecord {
  const journalState = value ?? {}
  const normalizedHead = Number(journalState.journalHead) || 0
  return {
    journalHead: normalizedHead,
    pendingOps: Number(journalState.pendingOps) || 0,
    lastAckedAt: typeof journalState.lastAckedAt === 'number' ? journalState.lastAckedAt : null,
    ackedOps: Array.isArray(journalState.ackedOps)
      ? journalState.ackedOps
          .filter((entry) => typeof entry === 'string' && entry.length > 0)
          .slice(-MAX_ACKED_KEYS)
      : [],
    pathHeads: journalState.pathHeads && typeof journalState.pathHeads === 'object'
      ? Object.fromEntries(
          Object.entries(journalState.pathHeads).filter(
            ([filePath, hash]) => typeof filePath === 'string' && typeof hash === 'string'
          )
        )
      : {},
    lastStateVector:
      typeof journalState.lastStateVector === 'number'
        ? journalState.lastStateVector
        : normalizedHead,
    lastPersistedAt:
      typeof journalState.lastPersistedAt === 'number' ? journalState.lastPersistedAt : null,
  }
}

function loadSyncStateFromSqlite(): boolean {
  const db = getSyncJournalDb()
  if (!db) return false
  ...
  try {
    ...
    for (const row of replicaRows) {
      ...
      syncJournalStateByProject.set(
        row.project_id,
        normalizeSyncJournalRecord({
          journalHead: row.replica_head,
    ...
    for (const [projectId, queue] of queuedOpsByProject.entries()) {
      const replica = getSyncJournalStateRecord(projectId)
      replica.pendingOps = queue.length
      replica.lastStateVector = replica.journalHead - replica.pendingOps
      syncJournalStateByProject.set(projectId, replica)
    }

    return replicaRows.length > 0 || opRows.length > 0
  } catch (error) {
    console.warn('[Sync] Failed to load sync journal state from SQLite:', error)
    return false
  }
}
...
function loadSyncStateFromJson(): boolean {
  try {
    ...
    const parsed = JSON.parse(fs.readFileSync(statePath, 'utf-8')) as {
      journalStates?: Record<string, SyncJournalStateRecord>
      queues?: Record<string, SyncOpRecord[]>
    }

    const journalStates = parsed.journalStates ?? {}
    const queues = parsed.queues ?? {}
    for (const [projectId, journalState] of Object.entries(journalStates)) {
      syncJournalStateByProject.set(projectId, normalizeSyncJournalRecord(journalState))
    }
    ...
    return Object.keys(journalStates).length > 0 || Object.keys(queues).length > 0
  } catch (error) {
    console.warn('[Sync] Failed to load sync journal state from JSON:', error)
    return false
  }
}
...
function persistSyncStateToJson(): void {
  try {
    const statePath = getSyncStatePath()
    const journalStates = Object.fromEntries(syncJournalStateByProject.entries())
    const queues = Object.fromEntries(queuedOpsByProject.entries())
    fs.writeFileSync(statePath, JSON.stringify({ journalStates, queues }, null, 2), 'utf-8')
  } catch (error) {
    console.warn('[Sync] Failed to persist sync journal state to JSON:', error)
  }
}
...
function persistProjectSyncStateToSqlite(projectId: string): boolean {
  ...
  try {
    ...
    console.warn(`[Sync] Failed to persist sync journal state to SQLite for ${projectId}:`, error)
    return false
  }
}

export function loadSyncState(): void {
  syncJournalStateByProject.clear()
  queuedOpsByProject.clear()

  const loadedFromSqlite = loadSyncStateFromSqlite()
  if (loadedFromSqlite) return

  loadSyncStateFromJson()
}

function persistSyncState(): void {
  const projectIds = Array.from(
    new Set<string>([
      ...syncJournalStateByProject.keys(),
