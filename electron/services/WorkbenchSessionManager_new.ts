...
function resolveRegisteredProjectPath(projectId: string): string | null {
  return normalizeWorkspaceId(readRegisteredProjectPath(projectId))
}
...
function resolveOwnedProjectPath(args: {
  ...
  const normalizedProjectPath = normalizeWorkspaceId(args.workspaceId)
...
function sanitizeSessionInput<T extends {
  ...
  return {
    ...input,
    workspaceId: resolveOwnedProjectPath({
...
function repairPersistedSessionState(state: PersistedWorkbenchSessionState): boolean {
  ...
  for (const [sessionKey, record] of Object.entries(state.sessions)) {
    ...
    const repairedRecord: PersistedWorkbenchSessionRecord = {
      ...record,
      workspaceId: ownedProjectPath ?? null,
    }
...
export class WorkbenchSessionManager extends EventEmitter<{
  ...
  private async reconcileSessionProjectPath(
    ...
    const normalizedNextProjectPath = this.normalizeWorkspaceId(nextProjectPath)
    if (record.workspaceId === normalizedNextProjectPath) {
      return
    }
    ...
    if (record.workspaceId && normalizedNextProjectPath && record.workspaceId !== normalizedNextProjectPath) {
      this.warnOwnershipMismatch("rebind_rejected", {
        sessionKey,
        projectId: record.projectId,
        laneId: record.laneId,
        currentProjectPath: record.workspaceId,
        requestedProjectPath: normalizedNextProjectPath,
      })
      return
    }

    if (record.workspaceId && normalizedNextProjectPath === null) {
    ...
    record.workspaceId = normalizedNextProjectPath
  ...
  async ensureSession(input: {
    ...
    await this.reconcileSessionProjectPath(sessionKey, record, sanitizedInput.workspaceId)
    this.persist()
  ...
  getTerminalBinding(input: {
    ...
    if (record.workspaceId && snapshot.workspaceId !== record.workspaceId) {
      this.warnOwnershipMismatch("terminal_path_mismatch", {
        ...
        tileId: input.tileId,
        terminalId,
        expectedProjectPath: record.workspaceId,
        actualProjectPath: snapshot.workspaceId,
      })
      delete record.terminalBindings[input.tileId]
      this.persist()
      this.emitState(sessionKey, record)
  ...
  async bindTerminal(input: {
    ...
    if (!record.workspaceId && snapshot?.workspaceId) {
      const registeredOwner = findRegisteredProjectPathOwner(snapshot.workspaceId)
      if (registeredOwner && registeredOwner.projectId !== input.projectId.trim()) {
        this.warnOwnershipMismatch("bind_terminal_rejected_foreign_owner", {
          ...
          tileId: input.tileId,
          terminalId: input.terminalId,
          terminalProjectPath: snapshot.workspaceId,
          owningProjectId: registeredOwner.projectId,
        })
        return this.emitState(sessionKey, record)
      }
