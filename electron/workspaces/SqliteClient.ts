// @ts-nocheck
/**
 * Workspace-catalog SQLite client.
 *
 * Identical to the assistant-runtime NodeSqliteClient except the service tag
 * is scoped to the workspace catalog so the two databases never share a
 * connection via Effect's dependency injection.
 */
...
export const TypeId: TypeId = "~local/sqlite-node/WorkspaceSqliteClient";
export type TypeId = "~local/sqlite-node/WorkspaceSqliteClient";

export const WorkspaceSqliteClient = ServiceMap.Service<Client.SqlClient>(
  "cozea/workspaces/SqliteClient",
);
...
const makeMemory = (
  ...
  makeWithDatabase(
    { ...config, filename: ":memory:", readonly: false },
    () => new DatabaseSync(":memory:", { allowExtension: config.allowExtension ?? false }),
  );
  ...
  Layer.effectServices(
    Config.unwrap(config)
      ...
      .pipe(
        Effect.flatMap(make),
        Effect.map((client) =>
          ServiceMap.make(WorkspaceSqliteClient, client).pipe(
