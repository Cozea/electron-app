...
- Browser-backed tiles are migrating from the renderer-wide T3 `<webview>` host to main-owned `WebContentsView` surfaces. `docs/browser-webcontentsview-migration-ledger.md` and `shared/browserSurfaceMigrationLedger.ts` record which family is on which backend; do not switch a family's backend without editing the ledger.
- Electron main owns every native browser view. Only main may create or destroy a `WebContentsView` for a first-party browser surface, and renderer code must never import or instantiate one. Renderer `<webview>` creation is forbidden outright once a family reaches `NATIVE_REQUIRED`.
- `runtimeTabId` is browser identity, not the DOM node. A React mount, unmount, Dockview move, title update, or visibility toggle must not create a second Chromium browser, and main must never hold two native views for one `runtimeTabId`. Hiding a surface does not destroy it.
- High-frequency browser geometry stays out of Zustand and persisted workbench stores. The renderer measures a DOM placeholder and publishes at most one bounds message per animation frame per surface, suppressing unchanged rectangles before IPC.
- Native browser views cannot be layered with CSS. Overlapping application overlays require native occlusion, and overlapping floating tiles require native child-view ordering that matches Dockview's visual order — `z-index` alone is never sufficient. Screenshot substitution and occlusion state are part of that model, not banned workarounds.
- A fallback browser host is still forbidden. The legacy and native hosts coexist only through explicit ledger states, never through a silent runtime fallback, and unsupported paths must fail with explicit diagnostics rather than a blank surface.
- T3 automation must resolve to the `webContents` of the exact view the user sees. Never introduce a hidden second browser for automation, and never let renderer code choose or report arbitrary `webContents` ids.
- Storage isolation survives the migration unchanged: global, workspace, and ephemeral browser scopes, published Org DevApp scope, and unpublished DevApp preview scope keyed by development source. DevApp worker bridges stay package-scoped.
- Application overlays share the semantic body-portal/layer contract in `docs/workbench-overlay-architecture.md`. Browser-owned presentation belongs to the active browser host (`HostedBrowserWebview` until a family cuts over); custom application overlays use `AppOverlayPortal` or its bounded anchored variant instead of raw global z-index values.
