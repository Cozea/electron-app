...
import {
  ...
  resolveEditorLaunch,
} from "../../../electron/assistant-runtime/open";
...
it.layer(NodeServices.layer)("isCommandAvailable", (it) => {
  it.effect("resolves win32 commands with PATHEXT", () =>
    Effect.gen(function* () {
      ...
      const dir = yield* fs.makeTempDirectoryScoped({ prefix: "cozea-open-test-" });
  ...
  it.effect("uses platform-specific PATH delimiter for platform overrides", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const firstDir = yield* fs.makeTempDirectoryScoped({ prefix: "cozea-open-test-" });
      const secondDir = yield* fs.makeTempDirectoryScoped({ prefix: "cozea-open-test-" });
...
it.layer(NodeServices.layer)("resolveAvailableEditors", (it) => {
  it.effect("returns installed editors for command launches", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const dir = yield* fs.makeTempDirectoryScoped({ prefix: "cozea-editors-" });
