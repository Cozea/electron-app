...
import type { ModelSelection, ProviderRuntimeEvent, ProviderSession } from "@cozea/assistant-contracts";
import {
  ApprovalRequestId,
  CommandId,
...
import { deriveServerPaths, ServerConfig } from "../../../../../electron/assistant-runtime/config.ts";
import { TextGenerationError } from "../../../../../electron/assistant-runtime/git/Errors.ts";
import { ProviderAdapterRequestError } from "../../../../../electron/assistant-runtime/provider/Errors.ts";
...
import { GitCore, type GitCoreShape } from "../../../../../electron/assistant-runtime/git/Services/GitCore.ts";
import { TextGeneration, type TextGenerationShape } from "../../../../../electron/assistant-runtime/git/Services/TextGeneration.ts";
...
import { ProviderCommandReactorLive } from "../../../../../electron/assistant-runtime/orchestration/Layers/ProviderCommandReactor.ts";
...
describe("ProviderCommandReactor", () => {
  ...
  it("generates a thread title on the first turn when the seeded title still matches", async () => {
    const harness = await createHarness();
    const now = new Date().toISOString();
  ...
  it("does not overwrite a custom thread title on the first turn", async () => {
    const harness = await createHarness();
    const now = new Date().toISOString();
    const seededTitle = "Please investigate reconnect failures after restar...";
