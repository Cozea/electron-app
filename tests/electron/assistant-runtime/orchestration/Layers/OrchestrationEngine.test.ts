...
import { PersistenceSqlError } from "../../../../../electron/assistant-runtime/persistence/Errors.ts";
...
} from "../../../../../electron/assistant-runtime/persistence/Services/OrchestrationEventStore.ts";
...
} from "../../../../../electron/assistant-runtime/orchestration/Services/ProjectionPipeline.ts";
...
async function createOrchestrationSystem() {
  const ServerConfigLayer = ServerConfig.layerTest(process.cwd(), {
    prefix: "cozea-orchestration-engine-test-",
  });
  ...
  it("stores completed checkpoint summaries even when no files changed", async () => {
    ...
    await system.run(
      engine.dispatch({
        ...
        checkpointRef: asCheckpointRef("refs/cozea/checkpoints/thread-turn-diff/turn/1"),
        status: "ready",
        files: [],
