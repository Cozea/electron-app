...
import type { ProviderKind, ProviderRuntimeEvent, ProviderSession } from "@cozea/assistant-contracts";
import {
  CommandId,
...
import { CheckpointStoreLive } from "../../../../../electron/assistant-runtime/checkpointing/Layers/CheckpointStore.ts";
import { CheckpointStore } from "../../../../../electron/assistant-runtime/checkpointing/Services/CheckpointStore.ts";
import { GitCoreLive } from "../../../../../electron/assistant-runtime/git/Layers/GitCore.ts";
import { CheckpointReactorLive } from "../../../../../electron/assistant-runtime/orchestration/Layers/CheckpointReactor.ts";
...
import { RuntimeReceiptBusLive } from "../../../../../electron/assistant-runtime/orchestration/Layers/RuntimeReceiptBus.ts";
...
import { checkpointRefForThreadTurn } from "../../../../../electron/assistant-runtime/checkpointing/Utils.ts";
...
function createGitRepository() {
  const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "cozea-checkpoint-handler-"));
...
describe("CheckpointReactor", () => {
  ...
  async function createHarness(options?: {
    ...
    const ServerConfigLayer = ServerConfig.layerTest(process.cwd(), {
      prefix: "cozea-checkpoint-reactor-test-",
    });
  ...
  it("continues processing runtime events after a single checkpoint runtime failure", async () => {
    const nonRepositorySessionCwd = fs.mkdtempSync(
      path.join(os.tmpdir(), "cozea-checkpoint-runtime-non-repo-"),
    );
