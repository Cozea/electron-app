...
import {
  ...
  type PtyProcess,
  type PtySpawnInput,
} from "../../../../../electron/assistant-runtime/terminal/Services/PTY";
import { TerminalManagerRuntime } from "../../../../../electron/assistant-runtime/terminal/Layers/Manager";
...
describe("TerminalManager", () => {
  ...
  it("filters app runtime env variables from terminal sessions", async () => {
    ...
    setEnv("PORT", "5173");
    setEnv("COZEA_ASSISTANT_PORT", "3773");
    ...
    try {
      ...
      expect(spawnInput.env.COZEA_ASSISTANT_PORT).toBeUndefined();
  ...
  it("injects runtime env overrides into spawned terminals", async () => {
    ...
    await manager.open(
      openInput({
        env: {
          COZEA_PROJECT_ROOT: "/repo",
          COZEA_WORKTREE_PATH: "/repo/worktree-a",
          CUSTOM_FLAG: "1",
        },
      }),
    );
    ...
    expect(spawnInput.env.COZEA_PROJECT_ROOT).toBe("/repo");
    expect(spawnInput.env.COZEA_WORKTREE_PATH).toBe("/repo/worktree-a");
