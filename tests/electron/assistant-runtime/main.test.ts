...
import { assistantRuntimeCli, CliConfig, recordStartupHeartbeat, type CliConfigShape } from "../../../electron/assistant-runtime/main";
import { ServerConfig, type ServerConfigShape } from "../../../electron/assistant-runtime/config";
...
it.layer(testLayer)("server CLI command", (it) => {
  it.effect("parses all CLI flags and wires scoped start/stop", () =>
    Effect.gen(function* () {
      ...
      assert.equal(resolvedConfig?.baseDir, "/tmp/cozea-cli-home");
      assert.equal(resolvedConfig?.stateDir, "/tmp/cozea-cli-home/dev");
  ...
  it.effect("uses env fallbacks when flags are not provided", () =>
    Effect.gen(function* () {
      yield* runCli([], {
        COZEA_ASSISTANT_MODE: "desktop",
        COZEA_ASSISTANT_PORT: "4999",
        COZEA_ASSISTANT_HOST: "100.88.10.4",
        COZEA_ASSISTANT_HOME: "/tmp/cozea-env-home",
        VITE_DEV_SERVER_URL: "http://localhost:5173",
        COZEA_ASSISTANT_NO_BROWSER: "true",
        COZEA_ASSISTANT_AUTH_TOKEN: "env-token",
      });
      ...
      assert.equal(resolvedConfig?.baseDir, "/tmp/cozea-env-home");
      assert.equal(resolvedConfig?.stateDir, "/tmp/cozea-env-home/dev");
  ...
  const openBootstrapFd = Effect.fn(function* (payload: Record<string, unknown>) {
    ...
    const filePath = yield* fs.makeTempFileScoped({ prefix: "cozea-bootstrap-", suffix: ".ndjson" });
  ...
  it.effect("recognizes bootstrap fd from environment config", () =>
    Effect.gen(function* () {
      ...
      yield* runCli([], {
        COZEA_ASSISTANT_MODE: "web",
        COZEA_ASSISTANT_BOOTSTRAP_FD: String(fd),
        COZEA_ASSISTANT_AUTH_TOKEN: "env-token",
        COZEA_ASSISTANT_NO_BROWSER: "true",
      });
  ...
  it.effect("uses bootstrap envelope values as fallbacks when CLI and env are absent", () =>
    Effect.gen(function* () {
      const fd = yield* openBootstrapFd({
        ...
        port: 4888,
        host: "127.0.0.2",
        assistantHome: "/tmp/cozea-bootstrap-home",
      ...
      assert.equal(resolvedConfig?.baseDir, "/tmp/cozea-bootstrap-home");
      assert.equal(resolvedConfig?.stateDir, "/tmp/cozea-bootstrap-home/dev");
      assert.equal(resolvedConfig?.devUrl?.toString(), "http://127.0.0.1:5173/");
      assert.equal(resolvedConfig?.noBrowser, true);
  ...
  it.effect("applies CLI then env precedence over bootstrap envelope values", () =>
    Effect.gen(function* () {
      ...
      yield* runCli(["--port", "4999", "--host", "0.0.0.0", "--auth-token", "cli-token"], {
        ...
        COZEA_ASSISTANT_HOME: "/tmp/cozea-env-home",
        COZEA_ASSISTANT_NO_BROWSER: "true",
        COZEA_ASSISTANT_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "true",
        COZEA_ASSISTANT_LOG_WS_EVENTS: "true",
      });
  ...
  it.effect("prefers --mode over COZEA_ASSISTANT_MODE", () =>
    Effect.gen(function* () {
  ...
  it.effect("prefers --no-browser over COZEA_ASSISTANT_NO_BROWSER", () =>
    Effect.gen(function* () {
      yield* runCli(["--no-browser"], {
        COZEA_ASSISTANT_NO_BROWSER: "false",
      });
  ...
  it.effect("supports CLI and env for bootstrap/log websocket toggles", () =>
    Effect.gen(function* () {
      yield* runCli(["--auto-bootstrap-project-from-cwd"], {
        COZEA_ASSISTANT_MODE: "desktop",
        COZEA_ASSISTANT_LOG_WS_EVENTS: "false",
        COZEA_ASSISTANT_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "false",
        COZEA_ASSISTANT_NO_BROWSER: "true",
      });
