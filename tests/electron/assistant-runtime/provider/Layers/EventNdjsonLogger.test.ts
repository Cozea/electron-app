...
import { makeEventNdjsonLogger } from "../../../../../electron/assistant-runtime/provider/Layers/EventNdjsonLogger.ts";
...
describe("EventNdjsonLogger", () => {
  it.effect("writes effect-style lines to thread-scoped files", () =>
    Effect.gen(function* () {
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "cozea-provider-log-"));
