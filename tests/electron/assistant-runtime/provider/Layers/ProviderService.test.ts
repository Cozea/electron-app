...
} from "../../../../../electron/assistant-runtime/provider/Errors.ts";
import type { ProviderAdapterShape } from "../../../../../electron/assistant-runtime/provider/Services/ProviderAdapter.ts";
import { ProviderAdapterRegistry } from "../../../../../electron/assistant-runtime/provider/Services/ProviderAdapterRegistry.ts";
import { ProviderService } from "../../../../../electron/assistant-runtime/provider/Services/ProviderService.ts";
import { ProviderSessionDirectory } from "../../../../../electron/assistant-runtime/provider/Services/ProviderSessionDirectory.ts";
import { makeProviderServiceLive } from "../../../../../electron/assistant-runtime/provider/Layers/ProviderService.ts";
import { ProviderSessionDirectoryLive } from "../../../../../electron/assistant-runtime/provider/Layers/ProviderSessionDirectory.ts";
import * as NodeServices from "@effect/platform-node/NodeServices";
import { ProviderSessionRuntimeRepositoryLive } from "../../../../../electron/assistant-runtime/persistence/Layers/ProviderSessionRuntime.ts";
import { ProviderSessionRuntimeRepository } from "../../../../../electron/assistant-runtime/persistence/Services/ProviderSessionRuntime.ts";
import {
...
it.effect("ProviderServiceLive keeps persisted resumable sessions on startup", () =>
  Effect.gen(function* () {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "cozea-provider-service-"));
...
it.effect(
  ...
  () =>
    Effect.gen(function* () {
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "cozea-provider-service-restart-"));
      const dbPath = path.join(tempDir, "orchestration.sqlite");
      const persistenceLayer = makeSqlitePersistenceLive(dbPath);
      const runtimeRepositoryLayer = ProviderSessionRuntimeRepositoryLive.pipe(
        Layer.provide(persistenceLayer),
...
routing.layer("ProviderServiceLive routing", (it) => {
  it.effect("routes provider operations, preserves stopped bindings, and recovers sendTurn", () =>
    Effect.gen(function* () {
      const provider = yield* ProviderService;
      const directory = yield* ProviderSessionDirectory;
      ...
      const persistedBinding = yield* directory.getBinding(session.threadId);
      ...
      if (Option.isSome(persistedBinding)) {
        ...
        assert.equal(persistedBinding.value.status, "stopped");
      }
  ...
  it.effect("reuses persisted resume cursor when startSession is called after a restart", () =>
    Effect.gen(function* () {
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "cozea-provider-service-start-"));
      const dbPath = path.join(tempDir, "orchestration.sqlite");
      const persistenceLayer = makeSqlitePersistenceLive(dbPath);
      const runtimeRepositoryLayer = ProviderSessionRuntimeRepositoryLive.pipe(
        Layer.provide(persistenceLayer),
