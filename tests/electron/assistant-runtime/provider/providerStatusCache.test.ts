...
} from "../../../../electron/assistant-runtime/provider/providerStatusCache.ts";
...
it.layer(NodeServices.layer)("providerStatusCache", (it) => {
  it.effect("writes and reads provider status snapshots", () =>
    Effect.gen(function* () {
      ...
      const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "cozea-provider-cache-" });
  ...
  it("ignores stale cached enabled state when the provider is now disabled", () => {
    ...
    const disabledFallback = makeProvider("codex", {
      ...
      status: "disabled",
      auth: { status: "unknown" },
      message: "Codex is disabled in Cozea settings.",
    });
