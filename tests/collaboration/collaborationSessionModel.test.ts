...
describe("collaboration session model", () => {
  ...
  it("allows a separately prepared binary-only publication at the same text barrier", () => {
    ...
    ).toMatchObject({ publishedCommitSha: PUBLISHED_SHA, publishedThroughSequence: 8 })
  })
})
