...
function message(text: string, id = "answer", streaming = true): ChatMessage {
  return { id: MessageId.makeUnsafe(id), role: "assistant", text, streaming, createdAt: "2026-09-05T00:00:00Z" }
}
...
describe("assistant text reveal", () => {
  it("smooths a burst, preserves prefixes, and stops frames when caught up", () => {
    ...
    step(250)
    expect(controller.getSnapshot("answer")).toEqual({ displayText: first.text + "B".repeat(100), isRevealing: false })
  ...
  it("honors original receipt times when React coalesces multiple updates", () => {
    ...
    step(10)
    expect(controller.getSnapshot("answer").displayText.length).toBeGreaterThanOrEqual(first.text.length)
    step(240)
  ...
  it("seeds a snapshot and reveals only its live tail even in the same React batch", () => {
    ...
    expect(controller.getSnapshot("answer")).toEqual({ displayText: "History.", isRevealing: true })
    expect(controller.consumeEntrance("answer")).toBe(false)
  })
  ...
  it("adopts authoritative replacements and shorter text immediately", () => {
    ...
    controller.sync([current])
    expect(controller.getSnapshot("answer")).toEqual({ displayText: current.text, isRevealing: false })
