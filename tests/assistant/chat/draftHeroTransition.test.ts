...
} from "@/features/assistant/chat/draftHeroTransition";

afterEach(() => {
  vi.unstubAllGlobals();
});

describe("resolveDraftHeroState", () => {
  it("returns true when backgroundSubmissionPending is true regardless of other flags", () => {
    expect(
      resolveDraftHeroState({
        isLocalDraftThread: false,
        hasTimelineEntries: true,
        isWorking: true,
        draftHeroDockRequested: true,
        backgroundSubmissionPending: true,
      }),
    ).toBe(true);
  });

  it("returns true for a local draft with no messages, not working, and no dock requested", () => {
    expect(
      resolveDraftHeroState({
  ...
  it("returns false if there are timeline entries", () => {
    expect(
      resolveDraftHeroState({
        isLocalDraftThread: true,
        hasTimelineEntries: true,
  ...
  it("returns false if assistant is working", () => {
    expect(
      resolveDraftHeroState({
        isLocalDraftThread: true,
        hasTimelineEntries: false,
  ...
  it("returns false if draft hero dock is requested", () => {
    expect(
      resolveDraftHeroState({
        isLocalDraftThread: true,
        hasTimelineEntries: false,
