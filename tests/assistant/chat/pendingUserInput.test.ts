...
it("keeps shortcuts scoped to visible non-editing cards and advances only single choices", () => {
  const context = { visible: true, responding: false, editing: false, modified: false };
  expect(pendingUserInputShortcutValue(nativeChoiceQuestion, "1", context)).toBe(" first\t");
  for (const blocked of [
    ...
    expect(
      pendingUserInputShortcutValue(nativeChoiceQuestion, "1", { ...context, ...blocked }),
    ).toBeNull();
  }
  expect(shouldAutoAdvancePendingUserInput(nativeChoiceQuestion)).toBe(true);
  expect(shouldAutoAdvancePendingUserInput(multiSelectQuestion)).toBe(false);
});
it("allows free text without options and rejects it when explicitly prohibited", () => {
  const question = { ...singleSelectQuestion, options: [] };
  expect(
    buildPendingUserInputAnswers([question], { scope: { customAnswer: "  Details  " } }),
  ).toEqual({ scope: "Details" });
  expect(
    buildPendingUserInputAnswers([{ ...question, allowCustomAnswer: false }], {
      scope: { customAnswer: "Details" },
    }),
  ).toBeNull();
});
