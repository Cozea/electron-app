...
describe("Worker capability table — coverage", () => {
  it("maps every method to a capability in the vocabulary", () => {
    for (const [method, capability] of Object.entries(DEV_APP_METHOD_CAPABILITIES)) {
      expect(ALL_DEV_APP_CAPABILITIES, `${method} maps outside the vocabulary`).toContain(capability)
    }
  })
...
describe("Worker message parsing — untrusted input", () => {
  it("parses a well-formed request", () => {
    expect(parseWorkerMessage({ kind: "request", id: "1", method: "project.readFile", params: { path: "a" } }))
      .toEqual({ kind: "request", id: "1", method: "project.readFile", params: { path: "a" } })
  })

  it("parses a response carrying a result", () => {
    expect(parseWorkerMessage({ kind: "response", id: "1", result: 7 }))
      .toEqual({ kind: "response", id: "1", result: 7 })
  })

  it("refuses a response carrying both a result and an error", () => {
    expect(parseWorkerMessage({ kind: "response", id: "1", result: 1, error: { code: "x", message: "y" } }))
      .toBeNull()
  })
  ...
  it("refuses over-long identifiers and method names", () => {
    expect(parseWorkerMessage({ kind: "request", id: "x".repeat(200), method: "project.readFile" })).toBeNull()
  ...
  it("does not vouch for a parsed method, only its shape", () => {
    ...
    const message = parseWorkerMessage({ kind: "request", id: "1", method: "definitely.not.a.method" })
