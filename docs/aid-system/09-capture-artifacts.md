...
Registry states: `starting`, `warm`, `retiring`, `stopped`, `failed`. Creation is admission before `starting`; native stream start/configuration calls are subordinate operations, not extra registry states. Reconfiguration creates an explicitly reserved generation. Retirement forbids new borrowers. Each asynchronous transition carries entry ID and generation so late completion cannot revive an evicted stream. Startup tasks are shared among concurrent readers of the same entry; cancellation of one reader does not cancel another owner's startup. A pending start itself holds a lifecycle reservation: zero frame borrowers cannot free an instance before its start callback settles. If retired meanwhile, stop that exact late-started stream, then free the reservation. Failed start releases its reservation without pretending a stream was stopped.
...
**CAP-01:** same stream survives 60 seconds of valid task activity without screenshots. **CAP-02:** no owner means deterministic stop; another owner's release does not interrupt remaining owners. **CAP-03:** capacity cannot silently evict an active stream. **CAP-04:** late startup/frame callback after reset is ignored and stopped. **CAP-05:** unchanged/idle evidence is distinguished from a new image and from stale evidence. **CAP-06:** crop transforms and mixed-scale capture match input geometry. **CAP-07:** exactly requested encodings/exports occur, none after ordinary actions. **CAP-08:** capture stop on revoke/sleep/host loss is measured. **CAP-09:** sharing UI never changes the bound source. **CAP-10:** slow consumer, OOM and encoder cancellation remain bounded. Gate G07 includes actual signed-app permissions and recorded stream starts/stops.
