...
Phase 4 review rework & packaged-daemon acceptance (2026-09-13T15:30Z): (1) Removed `daemonEnabled` and retired `VITE_FF_DAEMON_COLLABORATION` completely from the Session path (`useLiveSession`, `liveSessionModel`, `ProjectLayout`, `featureFlags`); session branches sync via `projectd` exclusively. (2) `resolveCollaborationGate` is now workspace-aware: immediately returns `session-daemon` for `ws_collab_*`, ensuring a Session Workbench never mounts the legacy in-app engine while the cloud sessions query is loading; regression added. (3) Executed the advance condition in `tests/collaboration/packagedDaemonDevRendererCutover.test.ts`: spawns the standalone compiled `projectd.mjs` bundle as an independent child process listening on domain socket; dev-renderer client connects and attaches session; peer connects to session room over network WebSocket; dev-renderer window closes/disconnects; daemon remains running independently; peer writes edit and daemon materializes it to disk without renderer; external disk write occurs and daemon publishes it to peer without renderer; reopened dev-renderer connects and confirms session remains live with all files. Full suite: 451 files / 3,220 passed / 5 skipped; all 5 typechecks + lint green. **Phase 4 officially closed; roadmap advances to Phase 5.**
