import { resolvePathLinkTarget } from "@/lib/terminalLinks";
...
function parseFileUrlHref(
  ...
  try {
    ...
    const path = options?.decodePath === false ? normalizedPath : safeDecode(normalizedPath);
...
/** Rewrite `file:` URLs to raw path+hash for ReactMarkdown `urlTransform` (matches t3). */
