...
function formatInviteeDisplayName(
  email: string,
...
function HeaderInboxButton() {
  ...
  const [activeTaskDismissId, setActiveTaskDismissId] =
    useState<Id<"projectTaskNotifications"> | null>(null);
  ...
  const formatActorName = useCallback(
    (
      actor:
        | {
            firstName?: string | null;
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        ...
                        <DropdownMenuItem
                          disabled={
                            joinLinkAction !== null || !canManageJoinLinks || !activeJoinLink
                          }
                          onClick={() => {
...
function HeaderProjectChangesButton({ projectId }: { projectId: Id<"projects"> | null }) {
...
export function UnifiedHeader({
  ...
  const recomputeVisibleBreadcrumbs = useCallback(() => {
    ...
    const separatorNode = measure.querySelector<HTMLElement>("[data-breadcrumb-measure-separator]");
    const ellipsisNode = measure.querySelector<HTMLElement>("[data-breadcrumb-measure-ellipsis]");
  ...
  return (
    <div
      className={cn(
        headerLayoutClassName,
        "h-10 flex items-center px-4 titlebar-drag-region transition-[padding] duration-200 ease-out",
        headerSurfaceClassName,
        className,
      )}
    >
      ...
      <div className="flex items-center w-full gap-3" style={headerContentStyle}>
        <div ref={breadcrumbContainerRef} className="flex min-w-0 flex-1 items-center gap-2">
          {breadcrumbs.length > 0 && (
            <>
              <div
                ref={breadcrumbViewportRef}
                className="titlebar-no-drag min-w-0 max-w-full overflow-hidden"
              >
                ...
                <Breadcrumb>
                  <BreadcrumbList className="flex-nowrap">
                    ...
                    {breadcrumbs.map((crumb, index) => (
                      ...
                      >
                        <span className="inline-block whitespace-nowrap">{crumb.label}</span>
                      </BreadcrumbItem>
                    ))}
                  </BreadcrumbList>
                </Breadcrumb>
