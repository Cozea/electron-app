...
function SpacingControl({
  ...
  const handleChange = (property: keyof ElementStyles, value: string) => {
    const cssValue = value ? `${value}px` : '0'
          ...
          return (
            <div key={side} className="space-y-0.5">
              <input
                ...
                min={0}
              />
              <span className="text-[10px] text-sidebar-foreground/60 text-center block">
                {side[0]}
              </span>
            </div>
          )
...
// Border radius control with 4 corners
    ...
    <div className="space-y-1.5">
      ...
      <div className="grid grid-cols-2 gap-1">
        {corners.map((corner) => {
          ...
          return (
            <div key={corner.label} className="flex items-center gap-1">
              <span className="text-[10px] text-sidebar-foreground/60 w-5">{corner.label}</span>
              <input
                type="number"
                value={value}
                onChange={(e) => handleChange(corner.prop, e.target.value)}
                className="flex-1 h-7 text-[11px] text-center px-1.5 bg-background/70 border border-sidebar-border/70 rounded-md font-mono focus:outline-none focus:ring-1 focus:ring-sidebar-ring/40"
                min={0}
              />
            </div>
          )
    ...
    >
      ...
      >
        ...
        <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as EditorTab)} className="flex-1 flex flex-col overflow-hidden">
          <TabsList className="w-full h-9 bg-transparent rounded-none p-0 border-b border-sidebar-border">
            <TabsTrigger
              value="styling"
              className={cn(
                'relative flex-1 h-9 rounded-none bg-transparent shadow-none text-[11px] font-medium',
            ...
            <ScrollArea className="flex-1">
              <div className="pb-6">
                {sections.showContent && shouldShowSection('Content') && selectedElement?.textContent !== undefined && (
                  ...
                  >
                    <textarea
                      className="w-full min-h-[70px] max-h-[160px] text-[11px] bg-background/70 border border-sidebar-border/70 rounded-md px-2 py-2 resize-y focus:outline-none focus:ring-1 focus:ring-sidebar-ring/40"
                ...
                {sections.showSize && shouldShowSection('Size') && (
                  <CollapsibleSection title="Size" icon={<Square className="h-3.5 w-3.5" />} defaultOpen>
                ...
                {sections.showLayout && shouldShowSection('Layout') && (
                  <CollapsibleSection title="Layout" icon={<LayoutGrid className="h-3.5 w-3.5" />} defaultOpen>
                    <div className="space-y-2">
                      <InlineSelectInput
                        label="Display"
                        property="display"
                ...
                {shouldShowSection('Spacing') && (
                  <CollapsibleSection title="Spacing" icon={<Move className="h-3.5 w-3.5" />} defaultOpen>
                ...
                {sections.showText && shouldShowSection('Text') && (
                  <CollapsibleSection title="Text" icon={<Type className="h-3.5 w-3.5" />} defaultOpen>
                    <div className="space-y-2">
                      <InlineTextInput label="Font" property="fontFamily" placeholder="Inter, system-ui" onPreview={handlePreview} />
                      <InlineInput label="Size" property="fontSize" onPreview={handlePreview} />
                      <div className="space-y-1.5">
                        <Label className="text-[10px] text-muted-foreground uppercase tracking-wider">
                          Style
                        </Label>
                        <div className="flex items-center gap-0.5 rounded-md p-0.5 w-fit border bg-sidebar-accent/60 border-sidebar-border/70">
                          <TextStyleToggle
                            icon={Bold}
                            property="fontWeight"
                            activeValue="bold"
                      ...
                      <InlineInput label="Line H" property="lineHeight" units={['', 'px', 'em', '%']} onPreview={handlePreview} />
                ...
                {shouldShowSection('Fill') && (
                  <CollapsibleSection title="Fill" icon={<Paintbrush className="h-3.5 w-3.5" />} defaultOpen>
                    <div className="space-y-2">
                      <InlineColorInput label="Color" property="backgroundColor" onPreview={handlePreview} />
                      <InlineTextInput label="Image" property="backgroundImage" placeholder="none / url(...)" onPreview={handlePreview} />
                      <InlineSelectInput
                        label="Size"
                ...
                {shouldShowSection('Stroke') && (
                  <CollapsibleSection title="Stroke" icon={<Square className="h-3.5 w-3.5" />} defaultOpen>
                    <div className="space-y-2">
                      <InlineInput label="Radius" property="borderRadius" onPreview={handlePreview} />
                      <BorderRadiusControl onPreview={handlePreview} />
                      <InlineInput label="Width" property="borderWidth" onPreview={handlePreview} />
                      <InlineSelectInput
                        label="Style"
                ...
                {shouldShowSection('Effects') && (
                  ...
                  >
                    <div className="space-y-2">
                      <InlineInput label="Opacity" property="opacity" units={['', '%']} onPreview={handlePreview} />
          ...
          <TabsContent value="events" className="flex-1 flex flex-col overflow-hidden m-0">
            <ScrollArea className="flex-1">
              <div className="p-3">
                <p className="text-xs text-muted-foreground">
                  Event handlers editing coming soon.
                </p>
              </div>
            </ScrollArea>
          </TabsContent>
