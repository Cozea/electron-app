...
export function WorkbenchSeamInsertion({
  ...
  }, [seamZones, hoveredZoneId])

  if (seamZones.length === 0) return null

  return (
    <>
      {seamZones.map((zone) => {
        const isHovered = armed && hoveredZoneId === zone.id

        return (
          <div
            key={zone.id}
            className={cn("absolute z-20", !armed && "pointer-events-none")}
            style={{
              left: zone.rect.x,
              top: zone.rect.y,
