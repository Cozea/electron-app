...
interface SidebarProjectTreeItemProps {
  ...
  onOpenLaneWorkbench: (
    project: SidebarProjectItem,
    laneId: string,
    options?: { openTile?: "assistantChat" | "browser" | "terminal" | "devServer"; focusTileId?: string },
  ) => Promise<void>
...
function normalizeExpandedLaneIdsByProjectId(
  ...
  return Object.fromEntries(
    Object.entries(value)
      .map(([projectId, laneIds]) => [projectId, dedupeStringArray(Array.isArray(laneIds) ? laneIds : [])] as const)
...
function readPersistedProjectSidebarState(): PersistedProjectSidebarState {
  ...
  try {
    ...
    return {
      expandedProjectIds: dedupeStringArray(Array.isArray(parsed.expandedProjectIds) ? parsed.expandedProjectIds : []),
      expandedLaneIdsByProjectId: normalizeExpandedLaneIdsByProjectId(parsed.expandedLaneIdsByProjectId),
      projectOrderIds: dedupeStringArray(Array.isArray(parsed.projectOrderIds) ? parsed.projectOrderIds : []),
    }
  } catch {
    return {
      expandedProjectIds: [],
...
function getTileCollections(workbench: WorkbenchProjectState | null | undefined): {
  ...
  return {
    agents: orderedTiles.filter((tile): tile is WorkbenchAssistantChatTile => tile.type === "assistantChat"),
...
function hasUnseenCompletion(thread: NonNullable<ReturnType<typeof useStore.getState>["threads"][number]>): boolean {
...
const SidebarProjectTreeItem = React.memo(function SidebarProjectTreeItem({
  project,
  projectIndex,
  projectCount,
  isCurrentProject,
  ...
  const collabBranch = React.useMemo(
    () => resolveProjectCollabBranch(project),
    [project],
  )
    ...
    <Collapsible open={isExpanded}>
      <div className="group/project-item flex items-center gap-1">
        <button
          type="button"
          className={cn(
            "flex min-w-0 flex-1 items-center gap-2.5 rounded-md px-2 py-1.5 text-left transition-colors hover:bg-accent hover:text-accent-foreground",
            activeSelectionLevel === "project" && "bg-accent text-accent-foreground",
          )}
          onClick={() => onToggleExpanded(project.id)}
        >
          <ProjectFavicon cwd={projectIconPath} />
          <div className="min-w-0 flex flex-1 items-center gap-1.5 overflow-hidden">
            <span className="min-w-0 truncate text-sm font-normal">{project.name}</span>
            <ChevronRight
              className={cn(
        ...
        <Button
          ...
          variant="ghost"
          size="icon"
          className="size-6 shrink-0 rounded-md p-0 text-muted-foreground/70 opacity-0 transition-opacity group-hover/project-item:opacity-100 group-focus-within/project-item:opacity-100 hover:bg-accent hover:text-foreground"
          ...
          {!shouldLoadLanes ? null : laneState === null ? (
            ...
            laneState.lanes.map((lane) => (
              <SidebarLaneRow
                ...
                activeTileId={lane.id === activeLane?.id && activeSelectionLevel === "tile" ? activeTileId : null}
...
const SidebarLaneRow = React.memo(function SidebarLaneRow(props: {
      ...
      <div className="group/lane-row space-y-1">
        <div className="flex items-center gap-1">
          <button
            ...
            className={cn(
              "flex min-w-0 flex-1 items-center gap-2.5 rounded-md px-2 py-1.5 text-left transition-colors hover:bg-accent hover:text-accent-foreground",
              props.isVisuallyActive && "bg-accent text-accent-foreground",
            )}
          ...
          >
            <div className="min-w-0 flex flex-1 items-center gap-1.5 overflow-hidden">
              ...
              <span
                className="inline-flex shrink-0 items-center text-sm text-muted-foreground/70"
          ...
          {props.onRefreshLaneState ? (
            <Button
              ...
              variant="ghost"
              size="icon"
              className="size-6 shrink-0 rounded-md p-0 text-muted-foreground/70 opacity-0 transition-opacity group-hover/lane-row:opacity-100 group-focus-within/lane-row:opacity-100 hover:bg-accent hover:text-foreground"
            ...
            <div className="space-y-1">
              ...
              {agents.map((tile) => (
                <button
                  ...
                  type="button"
                  className={cn(
                    "flex w-full items-center gap-2.5 rounded-md px-2 py-1.5 text-left transition-colors hover:bg-accent hover:text-accent-foreground",
                    props.activeTileId === tile.id && "bg-accent text-accent-foreground",
                  )}
                  onClick={() => {
                    void props.onOpenLaneWorkbench({ focusTileId: tile.id })
                  }}
                >
                  <ProviderGlyph provider={tile.provider} className="size-4 shrink-0 text-muted-foreground/75" />
                  <span className="min-w-0 flex-1 truncate text-sm">{tile.title}</span>
...
export function ProjectSidebar({
  ...
  const currentProjectId =
    providedProjectId ? String(providedProjectId) : currentProject?._id ? String(currentProject._id) : projectIdParam
  ...
  const [expandedLaneIdsByProjectId, setExpandedLaneIdsByProjectId] = React.useState<Record<string, string[]>>(
  ...
  const [projectPendingRename, setProjectPendingRename] = React.useState<SidebarProjectItem | null>(null)
  const [renameValue, setRenameValue] = React.useState("")
  const [renameError, setRenameError] = React.useState<string | null>(null)
  const [isRenamingProject, setIsRenamingProject] = React.useState(false)
  const [projectPendingDelete, setProjectPendingDelete] = React.useState<SidebarProjectItem | null>(null)
  ...
  React.useEffect(() => {
    ...
    const nextOrderedIds = buildOrderedProjects(projectItems, projectOrderIds).map((project) => project.id)
  ...
  const moveProject = React.useCallback((projectId: string, direction: "up" | "down") => {
    setProjectOrderIds((current) => {
      const ensuredOrder = buildOrderedProjects(projectItems, current).map((project) => project.id)
  ...
  const openLaneWorkbench = React.useCallback(
    async (
      ...
      const ensuredWorkbench =
        useProjectWorkbenchStore.getState().workbenches[buildWorkbenchScopeKey(project.id, laneId)] ?? null
      ...
      if (!options?.openTile && !options?.focusTileId && ensuredWorkbench) {
        ...
        if (!hasVisibleTiles) {
          const selectionTileId =
            ensuredWorkbench.order.find((tileId) => ensuredWorkbench.tiles[tileId]?.type === "selection") ?? null
  ...
  const currentWorkbenchPath = currentProjectId ? `${buildProjectPath(currentProjectId)}/workbench` : null
  const isOnCurrentProjectWorkbench = currentWorkbenchPath === location.pathname
  const currentSelectionLevel: "none" | "project" | "lane" | "tile" =
    !currentProjectId
      ? "none"
  ...
  const handleOpenProject = React.useCallback(
    async (project: SidebarProjectItem, localPath: string | null) => {
      try {
        ...
        primeLocalProjectPath(project.id, gitOpenResult.localPath, project.slug)
        navigate(buildProjectPath(project.id, "pages"), {
          state: {
            projectId: project.id,
            projectSlug: project.slug,
            projectName: project.name,
        ...
        const response = await window.electronAPI.dialog.showMessageBox({
          ...
          buttons: presentation.actionHref ? ["OK", presentation.actionLabel ?? "Open Settings"] : ["OK"],
          defaultId: 0,
          cancelId: 0,
  ...
  const handleConfirmDeleteProject = React.useCallback(
    async (confirmName: string) => {
      if (!projectPendingDelete || !convexUserId || isDeletingProject || confirmName !== projectPendingDelete.name) {
        return
      }
  ...
  return (
    <div className={cn("flex h-full", className)}>
      <div style={{ "--sidebar-width": "19rem" } as React.CSSProperties} className="h-full">
        <Sidebar
          collapsible="offcanvas"
          className="h-full w-[19rem] shrink-0 bg-content-surface"
          {...props}
        >
          ...
          <SidebarContent className="gap-0 px-2 py-3">
            <div className="mb-4 space-y-1 px-1">
              <button
                type="button"
                className="flex w-full items-center gap-2.5 rounded-md px-2 py-1.5 text-left transition-colors hover:bg-accent hover:text-accent-foreground"
                onClick={(event) => void openProjectCreationMenu(event)}
              >
                <SquarePen className="size-4 shrink-0 text-muted-foreground/75" />
                <span className="truncate text-sm font-normal">New project</span>
              </button>
              <button
                type="button"
                className="flex w-full items-center gap-2.5 rounded-md px-2 py-1.5 text-left transition-colors hover:bg-accent hover:text-accent-foreground"
              ...
              >
                ...
                <span className="min-w-0 flex-1 truncate text-sm font-normal">Automations</span>
              </button>
            </div>
            ...
            <div className="mb-3 flex items-center justify-between px-2">
              ...
              <div className="flex items-center gap-1">
                <Button
                  ...
                  variant="ghost"
                  size="icon"
                  className="size-7 rounded-lg p-0 text-muted-foreground/70 hover:bg-accent hover:text-foreground"
              ...
              {isProjectsLoading ? (
                ...
                sortedProjects.map((project, index) => (
                  <SidebarProjectTreeItem
                    ...
                    prefetchedLaneState={project.id === currentProjectId ? currentLaneState : undefined}
                    prefetchedActiveLane={project.id === currentProjectId ? currentActiveLane : undefined}
                    onRefreshActiveLaneState={
