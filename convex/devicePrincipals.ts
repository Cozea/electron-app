...
/** Nightly janitor for expired identity challenges, old security events, consumed grants, and expired DevApp uploads. */
export const cleanupExpiredIdentityState = internalMutation({
  args: {},
  handler: async (ctx) => {
    const now = Date.now()
    ...
    console.log(`[Maintenance] Cleaned ${oldChallenges.length + oldEvents.length + oldGrants.length} identity rows and ${expiredDevAppUploads.length} DevApp uploads`)
    return { deleted: oldChallenges.length + oldEvents.length + oldGrants.length + expiredDevAppUploads.length }
  },
})
