...
function toSessionDescriptor(
  ...
  const descriptor: CollaborationSessionDescriptor = {
    ...
    commitLeasePrincipalId: session.commitLeasePrincipalId
      ? String(session.commitLeasePrincipalId)
      : null,
...
function toParticipantDescriptor(
  ...
  return {
    ...
    principalId: String(participant.principalId),
...
async function getParticipant(
  ...
  return await ctx.db
    ...
    .withIndex("by_session_and_principal", (index) =>
      index.eq("sessionId", sessionId).eq("principalId", principalId),
    )
    .unique()
}
...
async function requireSessionAccess(
  ...
  if (!(await canAccessProject(ctx, session.projectId, principalId))) {
    throw new ConvexError("The authenticated principal cannot access this collaboration session")
  }
}
...
async function requireActiveEditorParticipant(
  ...
  const participant = await getParticipant(ctx, session._id, principalId)
  ...
  if (!(await canEditProject(ctx, session.projectId, principalId))) {
    throw new ConvexError("The authenticated principal cannot edit this project")
  }
  return participant
}
...
async function requireLeaseHolder(
  ...
  await requireActiveEditorParticipant(ctx, session, principalId)
  if (
    session.commitLeasePrincipalId !== principalId ||
    session.commitLeaseExpiresAt === undefined ||
    session.commitLeaseExpiresAt <= now
  ) {
    throw new ConvexError("The authenticated principal does not hold an active commit lease")
  }
}
...
async function requireRecordedLeaseHolder(
  ...
  await requireActiveEditorParticipant(ctx, session, principalId)
  if (session.commitLeasePrincipalId !== principalId) {
    throw new ConvexError("The authenticated principal does not own this commit preparation")
  }
}
...
async function upsertParticipant(
  ...
  const existing = await getParticipant(ctx, session._id, principalId)
  if (existing) {
...
export const createSession = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!(await canEditProject(ctx, args.projectId, user._id))) {
      throw new ConvexError("The authenticated principal cannot start collaboration for this project")
    }
    ...
    const sessionsForTarget = await ctx.db
      ...
      .collect()
    const existingActive = sessionsForTarget.find((session) => !isTerminalSession(session))
    if (existingActive) {
...
export const listForProject = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (!(await canAccessProject(ctx, args.projectId, user._id))) {
      throw new ConvexError("The authenticated principal cannot access this project")
    }
...
export const joinSession = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (requestedRole === "editor" && !canEdit) {
      throw new ConvexError("This principal may join the session only as an observer")
    }
...
export const heartbeatParticipant = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!participant || participant.leftAt !== undefined) {
      throw new ConvexError("The authenticated principal is not an active session participant")
    }
...
export const leaveSession = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (session.commitLeasePrincipalId === user._id && session.status !== "pushing") {
      await ctx.db.patch(session._id, {
        commitLeasePrincipalId: undefined,
...
export const acquireCommitLease = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const now = Date.now()
    const leaseIsActive =
      session.commitLeasePrincipalId !== undefined &&
      session.commitLeaseExpiresAt !== undefined &&
      session.commitLeaseExpiresAt > now
    if (leaseIsActive && session.commitLeasePrincipalId !== user._id) {
      throw new ConvexError({
        code: "commit_lease_held",
        message: "Another participant is preparing a collaboration commit",
        leaseUserId: session.commitLeasePrincipalId,
    ...
    const sameUserRenewal =
      leaseIsActive &&
      session.commitLeasePrincipalId === user._id &&
    ...
    await ctx.db.patch(session._id, {
      commitLeasePrincipalId: user._id,
...
export const advancePublishedBaseFromServer = mutation({
  args: {
    serverSecret: v.string(),
    sessionId: v.string(),
    publishedByPrincipalId: v.id("devicePrincipals"),
    commitSha: v.string(),
    coveredThroughSequence: v.number(),
  },
  handler: async (ctx, args) => {
    ...
    const next = advancePublishedCollaborationBase(
      toSessionDescriptor(session),
      {
        ...
        publishedByPrincipalId: String(args.publishedByPrincipalId),
        publishedAt,
      },
    )
    ...
    await recordEvent(ctx, updated, "base_advanced", {
      actorPrincipalId: args.publishedByPrincipalId,
...
export const closeSession = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await requireSessionAccess(ctx, session, user._id)
    const canClose =
      session.createdByPrincipalId === user._id ||
