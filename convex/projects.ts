...
const PROJECT_PURGE_FINAL_STAGE = 36
...
async function deleteProjectPurgeStage(
  ...
  switch (stage) {
    ...
    case 34: {
      const rows = await ctx.db.query("collaborationPublications").withIndex("by_project", q => q.eq("projectId", projectId)).take(PROJECT_PURGE_BATCH_SIZE)
      await deleteRows(ctx, rows)
      return rows.length
    }
    case 35: {
      const rows = await ctx.db.query("collaborationRepositoryResolutions").withIndex("by_project", q => q.eq("projectId", projectId)).take(PROJECT_PURGE_BATCH_SIZE)
      await deleteRows(ctx, rows)
      return rows.length
    }
    default:
