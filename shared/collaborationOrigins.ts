import type { FileChangeAttribution } from "./electronApiTypes"
...
export function isAttributedYjsOrigin(origin: unknown): origin is FileChangeAttribution {
  return typeof origin === "object" && origin !== null && typeof (origin as { origin?: unknown }).origin === "string"
}
...
export function resolveYjsOriginKind(origin: unknown): YjsOriginKind | string | undefined {
  ...
  if (isRemoteYjsOrigin(origin)) return extractRemoteYjsOrigin(origin)?.origin ?? "remote"
  if (isAttributedYjsOrigin(origin)) return origin.origin
  return undefined
}

export function extractAttributionOrigin(origin: unknown): FileChangeAttribution | null {
  if (!isAttributedYjsOrigin(origin)) return null
  const raw = origin as FileChangeAttribution
  return {
    origin: raw.origin,
    sourceOrigin: typeof raw.sourceOrigin === "string" ? raw.sourceOrigin : undefined,
    actorType: raw.actorType === "user" || raw.actorType === "agent" || raw.actorType === "system" ? raw.actorType : undefined,
...
export function extractRemoteYjsOrigin(origin: unknown): RemoteYjsOrigin | null {
  if (origin === "remote") return { type: "remote", origin: "remote" }
  if (!isRemoteYjsOrigin(origin)) return null
  const raw = extractAttributionOrigin(origin) ?? ({ origin: "remote" } as FileChangeAttribution)
  return { type: "remote", ...raw, origin: raw.origin ?? "remote" }
}
