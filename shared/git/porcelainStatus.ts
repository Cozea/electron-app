/**
 * The one parser for `git status --porcelain=v2 -z --branch`.
 *
 * There were two live status parsers: this one in projectd, and a porcelain v1
 * reader in the desktop's `checkpointOps`. The v1 reader took `R  <new>\0<old>`
 * to mean old-then-new, which is the order *without* `-z`, so the Changes list
 * showed every staged rename backwards. v2 names both paths unambiguously.
 *
 * Pure and string-only so it can run in projectd (Bun) and the Electron main
 * process alike. How many files to keep is the caller's policy, not the
 * parser's: pass `maxFiles` and read `isTruncated`.
 */
...
export function parsePorcelainV2Status(raw: string, maxFiles: number = MAX_STATUS_FILES): ParsedGitStatus {
