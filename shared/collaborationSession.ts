...
export interface CollaborationSessionDescriptor {
  ...
  commitLeasePrincipalId: string | null
...
export function validateCollaborationSession(
  ...
  requireCanonicalTrimmed(session.createdByPrincipalId, "Creating principal ID")
  ...
  const hasLease = session.commitLeasePrincipalId !== null
  if (hasLease) {
    requireCanonicalTrimmed(session.commitLeasePrincipalId!, "Commit lease user ID")
...
export function hasActiveCommitLease(
  ...
  return Boolean(
    session.commitLeasePrincipalId &&
...
export function advancePublishedCollaborationBase(
  ...
  const publishedByPrincipalId = requiredTrimmed(
    publication.publishedByPrincipalId,
    "Publishing principal ID",
  )
  const publishedAt = normalizeTimestamp(publication.publishedAt, "Publication time")

  if (
    session.commitLeasePrincipalId !== publishedByPrincipalId ||
