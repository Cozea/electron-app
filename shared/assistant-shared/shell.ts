...
const PATH_CAPTURE_START = "__COZEA_ASSISTANT_PATH_START__";
const PATH_CAPTURE_END = "__COZEA_ASSISTANT_PATH_END__";
...
export function mergePathEntries(
  ...
  const delimiter = platform === "win32" ? WINDOWS_PATH_DELIMITER : POSIX_PATH_DELIMITER;
  const merged: string[] = [];
  const seen = new Set<string>();
...
function envCaptureStart(name: string): string {
  return `__COZEA_ASSISTANT_ENV_${name}_START__`;
}

function envCaptureEnd(name: string): string {
  return `__COZEA_ASSISTANT_ENV_${name}_END__`;
}
