...
/**
 * Device-to-gateway request for a hosted published runtime.
 *
 * Image authority and registry credentials are intentionally absent. The gateway
 * resolves the exact release from Convex, verifies its signed attestation, and
 * performs the registry pull without trusting device-supplied image metadata.
 */
export interface DevAppHostedRuntimeStartRequest {
  runtimeId: string
  identity: DevAppRuntimeIdentity
  location: "hosted"
  state: Exclude<DevAppStateScope, "device">
  environment: Record<string, string>
  servicePort?: number
  network: boolean
  resources: DevAppRuntimeResources
...
/** Secret-free identity used to renew or release one hosted runtime lease. */
export interface DevAppHostedRuntimeControlRequest {
  runtimeId: string
  identity: DevAppRuntimeIdentity
  location: "hosted"
  state: Exclude<DevAppStateScope, "device">
...
export interface DevAppHostedRuntimeStartResponse {
  state: DevAppContainedRuntimeState
  transportUrl: string
  transportToken: string
  /** Per-client lease capability. Releasing one lease cannot stop another client. */
  controlToken: string
  serviceUrl: string | null
  /** Main-process-only capability for the hosted service reverse proxy. */
  serviceToken: string | null
}

export interface DevAppHostedRuntimeEvent {
  cursor: number
  envelope: DevAppContainedRuntimeTransportEnvelope
}

export interface DevAppHostedRuntimeEventsResponse {
  cursor: number
  events: DevAppHostedRuntimeEvent[]
}

export type DevAppContainedRuntimeStatus = "starting" | "running" | "stopping" | "stopped" | "failed"
...
export type DevAppContainerHelperTask = "status" | "start" | "stop" | "delete" | "cleanup" | "inspect" | "message"

export interface DevAppContainerCleanupRequest {
  publicationId?: string
...
export interface DevAppContainerHelperRequest {
  ...
  runtimeId?: string
  cleanup?: DevAppContainerCleanupRequest
...
export function devAppRuntimeAttestationPayload(attestation: DevAppRuntimeImageAttestation): string {
...
export function validateDevAppRuntimeReleaseImage(
  ...
  if (JSON.stringify(attestation.platforms) !== JSON.stringify(release.platforms)) {
    return "The DevApp runtime image platform statement is inconsistent."
  }
  return null
}

export function validateRuntimePlacement(location: DevAppRuntimeLocation, state: DevAppStateScope): string | null {
