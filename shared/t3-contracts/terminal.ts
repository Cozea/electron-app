...
const TerminalColsSchema = Schema.Int.pipe(Schema.greaterThanOrEqualTo(20)).check(
  Schema.lessThanOrEqualTo(400),
);
const TerminalRowsSchema = Schema.Int.pipe(Schema.greaterThanOrEqualTo(5)).check(
...
export const TerminalWriteInput = Schema.Struct({
  ...TerminalSessionInput.fields,
  data: Schema.String.check(Schema.pipe(Schema.nonEmptyString())).pipe(Schema.maxLength(65_536)),
});
...
export const TerminalSessionSnapshot = Schema.Struct({
  threadId: Schema.String.check(Schema.pipe(Schema.nonEmptyString())),
  terminalId: Schema.String.check(Schema.pipe(Schema.nonEmptyString())),
  cwd: Schema.String.check(Schema.pipe(Schema.nonEmptyString())),
...
const TerminalErrorEvent = Schema.Struct({
  ...
  message: Schema.String.check(Schema.pipe(Schema.nonEmptyString())),
});
