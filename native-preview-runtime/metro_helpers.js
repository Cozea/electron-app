...
const extensionLib = process.env.COZEA_NATIVE_PREVIEW_LIB_PATH;
...
function adaptMetroConfig(config) {
  ...
  config.serializer.processModuleFilter = (module) => {
    if (module.path === "__prelude__") {
      ...
      process.stdout.write(
        JSON.stringify({
          type: "COZEA_NATIVE_PREVIEW_expo_env_prelude_lines",
  ...
  // type definition files are often generated by plugins, they never impact the bundle though,
  ...
  config.resolver.extraNodeModules = {
    ...config.resolver.extraNodeModules,
    __COZEA_NATIVE_PREVIEW_lib__: extensionLib,
    __REACT_NATIVE_INTERNALS__: reactNativePath,
    __APPDIR__: appRoot,
    ...(process.env.COZEA_NATIVE_PREVIEW_STORYBOOK_CONFIG_PATH ? { __COZEA_NATIVE_PREVIEW_STORYBOOK_CONFIG_PATH__: process.env.COZEA_NATIVE_PREVIEW_STORYBOOK_CONFIG_PATH } : {}),
  };
  ...
  process.env.COZEA_NATIVE_PREVIEW_ORIG_BABEL_TRANSFORMER_PATH = config.transformer.babelTransformerPath;
  ...
  // being considered:
  if (process.env.COZEA_NATIVE_PREVIEW_DEV) {
    let origBlockList = [];
  ...
  process.stdout.write(
    JSON.stringify({
      type: "COZEA_NATIVE_PREVIEW_watch_folders",
      watchFolders: [config.projectRoot, ...config.watchFolders], // metro internally adds projectRoot as first entry to watch folders
    })
  );
  process.stdout.write("\n");

  config.cacheVersion = `COZEA_NATIVE_PREVIEW_metro_cache_version$${process.env.COZEA_NATIVE_PREVIEW_VERSION}$${config.cacheVersion}`;
