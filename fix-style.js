...
const newTabs = `        {sessions.length > 0 && (
          <div className="flex h-9 shrink-0 items-center overflow-x-auto border-b border-border bg-content-surface px-2 gap-1 scrollbar-none">
            {sessions.map((session) => {
              const isActive = activeSession?.terminalId === session.terminalId && !isCreatingNew
              const isLive = isInteractiveSession(session.terminal)
              const profile = AI_TERMINAL_PROFILES.find(p => p.id === session.profileId)
              ...
              return (
                ...
                >
                  <span className="flex min-w-0 items-center gap-2">
                    ...
                    <Icon className={cn("h-3.5 w-3.5 shrink-0 hidden sm:block", profile?.color)} />
                    <span className="truncate max-w-[140px] tracking-tight">{session.label}</span>
                  </span>
            ...
            <button
              ...
              className={cn(
                  "flex h-6 w-6 shrink-0 items-center justify-center rounded-full transition-[background-color,color]",
                  isCreatingNew
                      ? "bg-sidebar/90 dark:bg-secondary/80 text-sidebar-foreground dark:text-secondary-foreground"
                      : "text-muted-foreground hover:bg-foreground/5 hover:text-foreground"
              )}
                  ...
                  {AI_TERMINAL_PROFILES.map((profile) => {
                    ...
                    return (
                      <Button
                        key={profile.id}
                        variant="outline"
                        className="h-auto justify-start gap-2 px-3 py-2.5 text-left text-xs font-normal bg-content-surface hover:bg-accent hover:text-accent-foreground border-border/60"
                        disabled={!projectPath || launchingId !== null}
                        onClick={() => void launchAITerminal(profile)}
                      >
