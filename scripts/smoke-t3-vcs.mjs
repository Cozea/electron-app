#!/usr/bin/env bun
/**
 * Phase T5 smoke: T3 server serves vcs.refreshStatus via native Effect RPC.
 */
...
console.log(`[smoke-t3-vcs] starting shadow child on ${host}:${port}, T3 on ${t3Port}`);
...
const readyUrl = `http://${host}:${port}/.well-known/cozea/substrate/ready`;
...
try {
  ...
  const status = await callT3Unary(
    sessionPayload.baseUrl,
    sessionPayload.wsTicket,
    WS_METHODS.vcsRefreshStatus,
    { cwd: process.cwd() },
  );
  console.log("[smoke-t3-vcs] vcs.refreshStatus keys:", Object.keys(status ?? {}));
  if (!status || typeof status !== "object") {
    throw new Error("expected vcs.refreshStatus object");
  }

  console.log("[smoke-t3-vcs] ok");
} finally {
  child.kill("SIGTERM");
  await sleep(500);
