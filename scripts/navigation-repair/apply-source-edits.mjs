...
// Removed before the final PR is marked ready. It never invokes a shell or edits .git.
...
function safePath(relative) {
  if (typeof relative !== 'string' || !/^(apps|shared|tests|scripts|docs)\//.test(relative) || relative.split('/').includes('..')) {
    throw new Error(`Refusing source path: ${relative}`);
  }
...
for (const operation of batch.operations) {
  ...
  if (operation.expectedSha && (!previous || blobHash(previous) !== operation.expectedSha)) {
    throw new Error(`Unexpected source version: ${operation.path}`);
  }
  if (operation.kind === 'replace') {
    ...
    throw new Error(`Unknown source operation: ${operation.kind}`);
  }
